...
namespace gps {
	...
	void Model3D::ReadOBJ(std::string fileName, std::string basePath){
		...
		min = glm::vec4(10000.0f, 10000.0f, 10000.0f, 1.0f);
		max = glm::vec4(-10000.0f, -10000.0f, -10000.0f, 1.0f);
		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
				...
				for (size_t v = 0; v < fv; v++) {
					...
					if (vertexPosition.x < min.x) min.x = vertexPosition.x;
					if (vertexPosition.y < min.y) min.y = vertexPosition.y;
					if (vertexPosition.z < min.z) min.z = vertexPosition.z;
					if (vertexPosition.x > max.x) max.x = vertexPosition.x;
					if (vertexPosition.y > max.y) max.y = vertexPosition.y;
					if (vertexPosition.z > max.z) max.z = vertexPosition.z;
